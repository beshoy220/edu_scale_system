...
import 'package:edu_scale/teacher/assignments/presentation/providers/teacher_assignment_upload_provider.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

/// Entry point widget — wires up the [TeacherAssignmentUploadProvider]
/// for this editing session and hands off to the actual view.
class TeacherAddAssignmentPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => TeacherAssignmentUploadProvider(
        assignmentName: assignmentName,
        gradeId: selectedGradeClass.gradeId,
        classId: selectedGradeClass.classId,
        dueDate: dueDate,
      ),
      child: const _TeacherAddAssignmentView(),
    );
  }
}

class _TeacherAddAssignmentView extends StatelessWidget {
  const _TeacherAddAssignmentView();
  ...
  Future<void> _handleSave(BuildContext context) async {
    final provider = context.read<TeacherAssignmentUploadProvider>();
    ...
    Navigator.push(
      context,
      MaterialPageRoute<void>(builder: (context) => TeacherAssignmentsPage()),
    );
    ...
    if (provider.classId == null) {
      PushNotificationsService.sendNotification.sendByTopic(
        ...
        'Assignment',
        'A new assignment has been added! Click to view.',
      );
    } else {
      PushNotificationsService.sendNotification.sendByTopic(
...
class _Header extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final provider = context.watch<TeacherAssignmentUploadProvider>();
...
class _QuestionTitleField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final question = context
        .watch<TeacherAssignmentUploadProvider>()
        .currentQuestion;
    ...
    return ListView(
      ...
      children: [
        for (int i = 0; i < question.options.length; i++)
          Padding(
            ...
            child: _OptionPill(
              ...
              onTapCircle: () => context
                  .read<TeacherAssignmentUploadProvider>()
                  .setCorrectOption(i),
              onDelete: () => context
                  .read<TeacherAssignmentUploadProvider>()
                  .removeOption(i),
        ...
        if (question.options.length <
            TeacherAssignmentUploadProvider.maxOptions)
          _AddOptionPill(
            onTap: () {
              final error = context
                  .read<TeacherAssignmentUploadProvider>()
