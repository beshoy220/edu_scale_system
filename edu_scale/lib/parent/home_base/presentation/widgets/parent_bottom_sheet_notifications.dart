...
class ParentBottomSheetNotifications extends StatefulWidget {
  const ParentBottomSheetNotifications({super.key});

  @override
  State<ParentBottomSheetNotifications> createState() =>
      _ParentBottomSheetNotificationsState();
}

class _ParentBottomSheetNotificationsState
    extends State<ParentBottomSheetNotifications> {
  @override
  void initState() {
    super.initState();
    ...
    Future.microtask(() {
      if (!mounted) return;
      context.read<ParentNotificationProvider>().getLast50Notifications();
    });
  }

  @override
  Widget build(BuildContext context) {
    final provider = context.watch<ParentNotificationProvider>();
    return SizedBox(
      height: 500,
      child: Column(
        children: [
